...
class TestGenerateRandomAlbumView:
    ...
    def test_post_request_to_generate_random_album_view_returns_correct_status_code(self, client):
        url = reverse(viewname="album_generator_app:generate_random_album")
        drummer = Drummer.objects.create(name="John Bonham", bio="Legendary drummer of Led Zeppelin.")
        response = client.post(url, {"drummer": drummer.id})
        assert response.status_code == 200

    def test_generate_random_album_view_uses_correct_template(self, client):
        drummer = Drummer.objects.create(name="John Bonham", bio="Legendary drummer of Led Zeppelin.")
    ...
    def test_generate_random_album_view_returns_valid_album_data_in_context(self, client):
        drummer = Drummer.objects.create(name="John Bonham", bio="Legendary drummer of Led Zeppelin.")
