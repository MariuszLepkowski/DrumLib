...
def test_delete_comment_authenticated_author(client, user, drummer, album):
    comment = Comment.objects.create(author=user, text="Comment to delete", drummer=drummer, album=album)
...
def test_delete_comment_not_author(client, user, another_user, drummer, album):
    comment = Comment.objects.create(author=another_user, text="Comment to delete", drummer=drummer, album=album)
    client.force_login(user)
    url = reverse('comments_app:delete_comment', kwargs={'comment_id': comment.id})
    response = client.post(url)
