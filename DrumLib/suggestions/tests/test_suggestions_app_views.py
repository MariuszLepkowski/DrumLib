...
def test_suggest_drummer_post_valid(client, user):
    ...
    assert DrummerSuggestion.objects.filter(
        name="John Bonham", suggested_by=user
    ).exists()
...
def test_suggest_album_post_valid(client, user):
    ...
    assert response.status_code == 302  # Redirect to thank you page
    assert AlbumSuggestion.objects.filter(
        album_title="Led Zeppelin IV", suggested_by=user
    ).exists()
