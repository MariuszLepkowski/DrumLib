...
class TestEditProfileView:
    ...
    def test_edit_profile_view_accessible_for_logged_in_user(self, client, logged_in_user):
        url = reverse('user_management_app:edit_profile')
    ...
    def test_edit_profile_view_uses_correct_template(self, client, logged_in_user):
        url = reverse('user_management_app:edit_profile')
        response = client.get(url)
        assert 'user_management_app/edit_profile.html' in [t.name for t in response.templates]
